...
Simulations to examine the effects of predatory density/attack rate, size-selectivity, & preference for infected hosts on schisto transmission potential in a seasonal scenario.
...
parms["ENV"] = 500 # Units: L
parms["r"] = 0.25   # Units: day-1
parms["step"] = 1  # Units: day
parms["epsilon"] = 20 # Units: L host-1, day-1 (Rounded estimate from Civitello and Rohr)
parms["sigma"] = 0.5 # Units: - (Rounded estimate from Civitello and Rohr)
parms["m_M"] = 1   # Units: day-1
parms["m_Z"] = 1   # Units: day-1
parms["M_in"] = 10 # Units: day-1
parms["K"] = 5
parms["Det"] = 0 # Units mg C L-1 d-1 (detritus)
...
# Adds parameters for a generalist (growing) biocontrol predator
parms["grow_out"] = TRUE
parms["L0"] = 40
parms["Linf"] = 213.63
parms["kmax"] = 0.371/30 #3.19/365
parms["gamma"] = 5e-6
parms["ap"] = -2.6132
parms["bp"] = 3.5502
parms["mup"] = 2.21/365
...
parms["pred_N"] = 2.5 # predator abundance (so predator density = pred_N/ENV = #/L)
parms["pred_eps"] = 1 # relative predator search inefficiency in the wild vs lab
parms["pred_a"] = 50/225/parms["ENV"]/parms["pred_eps"] # maximum predator attack rate; Units = ENVs swept per day, est = 50L swept / day or 8.33 * 6 L tanks (from sokolow etal 2014 acta tropica; occurs at Prawn-to-snail biomass ratio of 225)
parms["pred_hm"] = 0.612 # pred handling time as a power-law function of body mass (Sokolow et al. 2014, acta tropica) - intercept
parms["pred_hg"] = -0.805# pred handling time as a power-law function of body mass (Sokolow et al. 2014, acta tropica) - exponent
parms["pred_window"] = TRUE # Is size selective predation defined by a vulnerable window (FALSE = continuous size selectivity)
parms["gape_min"] = 0 # minimum size that can be consumed by predator
parms["gape_max"] = 1/(3*5) #maximum biomass ratio that can be consumed by predator (from Sokolow et al. 2014, acta tropica and converting our dry-mass estimate to their wet mass estimate ~5-fold diff)
parms["pred_size_gradient"] = -0.237 # continuous size-selectivity effect (parameterized by Sokolow et al. 2014 acta tropica)
parms["pred_alpha_Inf"] = 0 # Predator preference to attack infected individuals, must be >= -1 (0 = no preference)
```
...
```{r generalist_predator_dynamic}
gen_pred_dynamic =function(t, y, parameters) { 
  L=y[1]; P=y[2]
  with(as.list(parameters),{ 
    B = 10^(ap)*(L/10)^bp # Converts prawn length from mm->cm-> g
    OMEGA = B*P
    kstar = kmax/(1 + gamma*OMEGA)
    dLdt = kstar*(Linf - L)
    dPdt = -P*(mup*B^d + omega*OMEGA)
    result = c(dLdt,dPdt)          # The function for simulation expects us to store the result this way
    return(list(result))
  }
  ) 
} 
...
pred_growth = lsoda(y = c(L = 40, P = 2.5), times = 1:parms["timespan"], func=gen_pred_dynamic, parms = parms)
...
run_DEB_IBM = function(N.snail=60, min.L=2, max.L=16, Food=1, drip=T, pars){
  ...
  # Set up predators and/or grow-out
  if(pars["grow_out"] == TRUE){
    #can run the whole thing at beginning because no feedbacks from snails (assumption of generalism)
    pred_growth = lsoda(y = c(L = as.numeric(pars["L0"]), P = as.numeric(pars["pred_N"])), times = 0:pars["timespan"], 
                        func=gen_pred_dynamic, parms = pars)
    state$Pred_L = pred_growth[,"L"]
    state$Pred_N = pred_growth[,"P"]
  }else{ # If grow-out does not occur, then keep predator size and density constant
    state$Pred_L = rep(as.numeric(pars["L0"]), times=pars["timespan"]+1) # Lines up to dataframe, which is timespan + 1 long
    state$Pred_N = rep(as.numeric(pars["pred_N"]), times=pars["timespan"]+1) # Lines up to dataframe, which is timespan + 1 long
  }
  ...
  for(tick in 1:pars["timespan"]){
    ...
    if(dim(state$Snails[[tick]])[1] > 0){
      ...
      # Determine snail mortality rates due to size- and infection-selective predation
        # Calculate average biomass of predators Units g WET weight
        # Note that "AppMass" is the snail's DRY WEIGHT in mg 
          pred_M = 10^(pars["ap"])*(state$Pred_L[tick]/10)^pars["bp"]
          pred_ht = pars["pred_hm"]*pred_M^pars["pred_hg"]
        # Prawn-to-snail biomass ratios
          P_S_ratio = pred_M/(snail.stats[,"Appmass"]*5/1000) # *5/1000 to go from mg DRY to g WET
          pred_a = pars["pred_a"]*P_S_ratio  
      if(pars["pred_window"] == TRUE){
         hb = as.numeric(pars["hb"]) + (pred_a*(1 + (snail.stats[,"Cercs"]>0)*pars["pred_alpha_Inf"])*state$Pred_N[tick]) / 
                                  (1 + pred_a * pred_ht * Uninf.snails + 
                                     pred_a*(1 + pars["pred_alpha_Inf"]) * pred_ht * Inf.snails)

        # hb = as.numeric(pars["hb"]) + (pars["pred_a"]*(1 + (snail.stats[,"Cercs"]>0)*pars["pred_alpha_Inf"]) * state$Pred_N[tick]) / (1 + pars["pred_a"] * pars["pred_h"] * Uninf.snails + pars["pred_a"] *(1 + pars["pred_alpha_Inf"]) * pars["pred_h"] * Inf.snails) * 
...
Parameters_of_Interest = c("pred_N", "L0")
...
facsim_result = DEBIBM_factorial_simulator(OoI=Outputs_of_Interest, PoI=Parameters_of_Interest, mins = c(0, 40), maxs = c(10, 200), levels = 10, iterations=8)

facsim_Summary = aggregate(Cercariae ~ pred_N*L0, FUN=mean, data=facsim_result)
 
ggplot(data=facsim_Summary, aes(x=pred_N, y=Cercariae, group=as.factor(L0), color=as.factor(L0))) + geom_point() + geom_line()
# 
facsim_Summary2 = aggregate(Snails ~ pred_N*L0, FUN=mean, data=facsim_result)
ggplot(data=facsim_Summary2, aes(x=pred_N, y=Snails, group=as.factor(L0), color=as.factor(L0))) + geom_point() + geom_line()
...
parms["pred_N"] = 0
# pars["pred_alpha_Inf"] = 0
rep_states_NoPred = replicate(n=3, summarized_DEB_IBM(drip=T), simplify=T)
parms["pred_N"] = 5
rep_states_Pred = replicate(n=3, summarized_DEB_IBM(drip=T), simplify=T)

```

```{r plotting}
FigS1A_df = data.frame(rbind(summarized_IBM_reps(rep_states_NoPred, "Snails"), summarized_IBM_reps(rep_states_Pred, "Snails")), "M_in" = rep(c("Constant", "Biweekly pulse"), each=151))
...
FigS1B_df = data.frame(rbind(summarized_IBM_reps(rep_states_NoPred, "Cercariae"), summarized_IBM_reps(rep_states_Pred, "Cercariae")), "M_in" = rep(c("constant", "biweekly pulse"), each=151))
#### Set up means and CIs for supplemental figure ####
...
FigS1C_df = data.frame(rbind(summarized_IBM_reps(rep_states_NoPred, "Food"), summarized_IBM_reps(rep_states_Pred, "Food")), "M_in" = rep(c("constant", "biweekly pulse"), each=151))
#### Set up means and CIs for supplemental figure ####
...
FigS1D_df = data.frame(rbind(summarized_IBM_reps(rep_states_NoPred, "Miracidia"), summarized_IBM_reps(rep_states_Pred, "Miracidia")), "M_in" = rep(c("constant", "biweekly pulse"), each=151))
#### Set up means and CIs for supplemental figure ####
