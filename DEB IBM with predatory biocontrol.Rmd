...
parms["pred_N"] = 0 # predator abundance
parms["pred_a"] = 0.1 # predator attack rate: 50L swept / day or 8.33 * 6 L tanks (from sokolow etal 2014 acta tropica)
...
parms["gape_max"] = 100 #maximum size that can be consumed by predator
parms["pred_size_gradient"] = -0.237 # continuous size-selectivity effect (parameterized by Sokolow et al. 2014 acta tropica)
parms["pred_alpha_Inf"] = 2 # Predator preference to attack infected individuals, must be non-negative (1 = no preference)
```
...
Initialize_IBM = function(N.snail, min.L, max.L, Food, pars=parms){
  ...
  list("Snails" = list(snail.stats), "Env_F" = Food, "Env_M" = 0, "Env_Z" = 0, "Env_G" = 0, "Parameters" = pars)
}
```

```{r simulate_IBM}

run_DEB_IBM = function(N.snail=60, min.L=2, max.L=16, Food=1, n.ticks=112, drip=T, pars=parms){
  ...
  for(tick in 1:n.ticks){
    ...
    if(dim(state$Snails[[tick]])[1] > 0){
      ...
      if(pars["pred_window"] == TRUE){
        hb = as.numeric(pars["hb"]) + (pars["pred_a"]*(1 + (snail.stats[,"Cercs"]>0)*pars["pred_alpha_Inf"]) * pars["pred_N"]) / (1 + pars["pred_a"] * pars["pred_h"] * Uninf.snails + pars["pred_a"] *(1 + pars["pred_alpha_Inf"]) * pars["pred_h"] * Inf.snails) * 
...
cumulative_DEB_IBM = function(N.snail=60, min.L=2, max.L=16, Food=1, n.ticks=112, drip=T, pars){
  cumulative_totals(run_DEB_IBM(N.snail, min.L, max.L, Food, n.ticks, drip, pars))
}


Outputs_of_Interest = c("Snails", "DEBmass", "Appmass", "Food", "Miracidia", "Cercariae", "Eggs")
Parameters_of_Interest = names(parms)[c(38,43)]
...
DEBIBM_factorial_simulator = function(OoI, PoI, mins, maxs, levels, iterations, pars=parms){ # Factorial simulator for cumulative totals
  ...
  # returns back just the Outputs of interest, but sends back all pars for safety
  subset(reps, select=c(OoI, PoI)) 
  
}

facsim_result = DEBIBM_factorial_simulator(OoI=Outputs_of_Interest, PoI=Parameters_of_Interest, mins = c(0,0), maxs = c(5,50), levels = 5, iterations=10)
