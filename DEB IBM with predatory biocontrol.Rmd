...
parms["pred_a"] = 50/parms["ENV"] # predator attack rate; Units = ENVs swept per day, est = 50L swept / day or 8.33 * 6 L tanks (from sokolow etal 2014 acta tropica)
parms["pred_h"] = 0.1 # pred handling time # 0.1 = 10 snails per day
...
parms["pred_alpha_Inf"] = 1 # Predator preference to attack infected individuals, must be >= -1 (0 = no preference)
```
  ...
  for(tick in 1:pars["timespan"]){
    ...
    if(dim(state$Snails[[tick]])[1] > 0){
      ...
      # Determine snail mortality rates due to size- and infection-selective predation
        # Calculate average biomass of predators
          pred_M = 10^(pars["ap"])*(state$Pred_L[tick]/10)^pars["bp"]
      if(pars["pred_window"] == TRUE){
         hb = as.numeric(pars["hb"]) + (pars["pred_a"]*(1 + (snail.stats[,"Cercs"]>0)*pars["pred_alpha_Inf"])*state$Pred_N[tick]) / 
                                  (1 + pars["pred_a"] * pars["pred_h"] * Uninf.snails + 
                                     pars["pred_a"]*(1 + pars["pred_alpha_Inf"]) * pars["pred_h"] * Inf.snails) *
                            (snail.stats[,"Appmass"] >= pars["gape_min"]*pred_M & # is the snail "in the window"
                                snail.stats[,"Appmass"] <= pars["gape_max"]*pred_M)
...
facsim_result = DEBIBM_factorial_simulator(OoI=Outputs_of_Interest, PoI=Parameters_of_Interest, mins = c(0, 40), maxs = c(10, 200), levels = 10, iterations=4)
...
# 
# facsim_Summary2 = aggregate(DEBmass ~ pred_N, FUN=mean, data=facsim_result)
# 
# ggplot(data=facsim_Summary2, aes(x=pred_N, y=DEBmass)) + geom_point() 


#ggplot(data=facsim_Summary, aes(x=pred_N, y=Cercariae, color=as.factor(gape_max), group=as.factor(gape_max))) + geom_point() 
...
summarized_DEB_IBM = function(N.snail=60, min.L=2, max.L=16, Food=1, drip=T){
  digest_state(run_DEB_IBM(N.snail, min.L, max.L, Food, drip))
}
...
rep_states_NoPred = replicate(n=10, summarized_DEB_IBM(drip=T), simplify=T)
pars["pred_N"] = 10
rep_states_Pred = replicate(n=10, summarized_DEB_IBM(drip=T), simplify=T)
